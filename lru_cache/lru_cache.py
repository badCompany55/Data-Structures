...
#   # max number of nodes it can hold
#   # current number of nodes it is holding
#   # Hold key-value entries in order
#   # Find the least recently used entry and delete it
# #     DLL: remove from tail
# #     Array: pop()
# #     Array: add to back, remove from front: unshift()
...
#       value = None
#
#       if key in self.fastAccess:
#           print(key)
#           node = self.fastAccess[key]
#           value = node.value[1]
#           self.order.move_to_front(node)
#
#       print(value)
...
#   def set(self, key, value):
#       if key in self.fastAccess:
#           # need to over ride value in DLL
#           # move to front since its the most recently used
#           node = self.fastAccess[key]
#           self.order.move_to_front(node)
#           node.value = (key, value)
#       else:
#           if self.current_nodes == self.max_number_of_nodes:
#               # self.fastAccess.pop(self.order.tail.value[0])
#               del self.fastAccess[self.order.tail.value[0]]
#               self.order.remove_from_tail()
#               self.current_nodes -= 1
#
#           self.order.add_to_head((key, value))
#           self.fastAccess[key] = self.order.head
#           self.current_nodes += 1
...
# self.assertEqual(self.test.get('item1'), 'a')
test.set('item4', 'd')
test.get('item1')
